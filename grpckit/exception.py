...
class RpcException(Exception):
    ...
    def status_string(self):
        ...
        Example:
            >>> RpcException(status_code=StatusCode.NOT_FOUND).status_string
            'NOT_FOUND'
        """
        return self.status_code.name


class Aborted(RpcException):
...
class AlreadyExists(RpcException):
...
class FailedPrecondition(RpcException):
...
class InvalidArgument(RpcException):
...
class ResourceExhausted(RpcException):
...
class Unauthenticated(RpcException):
...
class Unimplemented(RpcException):
